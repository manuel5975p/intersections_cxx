#ifndef TRIABLE_ABB_HPP
#define TRIABLE_ABB_HPP
#include "structs.hpp"
struct Point3_dontuse{
    float           x;
    float           y;
    float           z;
...
#define SIGN3( A ) \
  ((((A).x < EPSILON) ? 4 : 0) | (((A).x > -EPSILON) ? 32 : 0) | \
   (((A).y < EPSILON) ? 2 : 0) | (((A).y > -EPSILON) ? 16 : 0) | \
   (((A).z < EPSILON) ? 1 : 0) | (((A).z > -EPSILON) ? 8 : 0))
...
bool intersect(const triangle<float>& t, const aabb<float>& b){
    ...
    for(size_t i = 0;i < 3;i++){
        ex(i) = 1.0 / ex(i);
        copy.v1(i) = (t.vertices[0](i) - b.start(i)) * ex(i) - 0.5;
        copy.v2(i) = (t.vertices[1](i) - b.start(i)) * ex(i) - 0.5;
        copy.v3(i) = (t.vertices[2](i) - b.start(i)) * ex(i) - 0.5;
    }
    return t_c_intersection(copy) == 0;
}
/*___________________________________________________________________________*/

/* Which of the six face-plane(s) is point P outside of? */

long face_plane(Point3_dontuse p)
{
long outcode;
...
long bevel_2d(Point3_dontuse p)
{
long outcode;
...
long bevel_3d(Point3_dontuse p)
{
long outcode;
...
long check_point(Point3_dontuse p1, Point3_dontuse p2, float alpha, long mask)
{
Point3_dontuse plane_point;
...
long check_line(Point3_dontuse p1, Point3_dontuse p2, long outcode_diff)
{
...
long point_triangle_intersection(Point3_dontuse p, Triangle3_dontuse t)
{
long sign12,sign23,sign31;
Point3_dontuse vect12,vect23,vect31,vect1h,vect2h,vect3h;
Point3_dontuse cross12_1p,cross23_2p,cross31_3p;
...
float d,denom;
Point3_dontuse vect12,vect13,norm;
Point3_dontuse hitpp,hitpn,hitnp,hitnn;
...
/* Do this for all four diagonals.                                */
   ...
   if(fabs(denom=(norm.x + norm.y + norm.z))>EPSILON)
